
import { Link } from "react-router-dom";

export default function Footer() {
    const currentYear = new Date().getFullYear();

    return (
        <footer className="w-full border-t border-white/5 bg-obsidian/50 backdrop-blur-md py-10 mt-auto glass">
            <div className="container mx-auto px-6 flex flex-col items-center justify-center gap-4">

                <p className="text-gray-500 text-sm tracking-tight order-2 md:order-1">
                    © {currentYear} <span className="text-white font-medium">Sibtain Ali Raza</span>. All rights reserved.
                </p>
                <div className="flex items-center gap-8 text-gray-400 text-sm order-1 md:order-2">


                    <a
                        href="mailto:contact@sibtainaliraza.com"
                        className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center hover:bg-[var(--color-electric-teal)] hover:shadow-[0_0_15px_var(--color-electric-teal)] transition-all duration-300"
                        title="Email Me"
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" />
                        </svg>
                    </a>

                    <a
                        href="https://github.com/sibtainaliraza"
                        target="_blank"
                        rel="noreferrer"
                        className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center hover:bg-[var(--color-electric-teal)] hover:shadow-[0_0_15px_var(--color-electric-teal)] transition-all duration-300"
                        title="GitHub"
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                        </svg>
                    </a>

                    <a
                        href="https://linkedin.com/in/sibtain-ali-raza"
                        target="_blank"
                        rel="noreferrer"
                        className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center hover:bg-[var(--color-electric-teal)] hover:shadow-[0_0_15px_var(--color-electric-teal)] transition-all duration-300"
                        title="LinkedIn"
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                        </svg>
                    </a>
                </div>

                <Link
                    to="/admin/login"
                    className="w-1.5 h-1.5 rounded-full bg-white/5 hover:bg-[var(--color-bg-electric-teal)] transition-all duration-500"
                    title="Admin Dashboard"
                />
            </div>
        </footer>
    );
}